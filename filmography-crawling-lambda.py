...
#*****************************************
	...
	#return dump_string
	#s3 = boto3.client('s3')
	#s3.put_object(Bucket = 'selenium-result-for-filmography-bucket', Key = 'id값.json', Body = dump_string)
...
def delete_sele_input_bucket(id):
...
def delete_sele_output_bucket(id):
	s3_client = boto3.client('s3')
	bucket = 'selenium-result-for-filmography-bucket'
...
def lambda_handler(event, context):
	...
	id = ''
	if event :
		id =str(event["Records"][0]["s3"]["object"]["key"]) # id.txt 로 읽음
		splt = id.split(".")
		splt_id = splt[0] # only id 값 


	delete_sele_output_bucket(splt_id + '.json')


	s3 = boto3.client('s3')
	file_obj = s3.get_object(Bucket="celebrity-names-for-selenium-bucket", Key = id)
	file_content = file_obj["Body"].read().decode("utf-8")
	cele_name = file_content.split("/")
	del cele_name[0]
	#print(cele_name)
	...
	#dump_string = json.dumps(gv_str, indent = 4, ensure_ascii = False)
	s3.put_object(Bucket = 'selenium-result-for-filmography-bucket', Key = splt_id + '.json', Body = dump_string)

	delete_sele_input_bucket(id)
